 ...
 * @property {(() => string[]|Promise<string[]>)|undefined} [getDeferredSessionIds] - 尚未安全 materialize 的会话；push/status/pull 前置镜像必须保护它们。
 * @property {((result: object) => void|Promise<void>)|undefined} [onRestored] - materialize 后的宿主协调回调。
 * @property {((ctx: {repoDir: string, mirrorDir: string}) => boolean|{changed?: boolean}|Promise<boolean|{changed?: boolean}> )|undefined} [prepareMirrorMetadata] - commit 前把宿主元数据投影进 mirror carrier。
 * @property {((ctx: {repoDir: string, mirrorDir: string}) => void|Promise<void>)|undefined} [applyMirrorMetadata] - restore 后把 mirror carrier 应用回宿主。
...
export class SyncEngine {
   ...
   * @returns {Promise<{deviceWritten: boolean, metadataChanged: boolean, mirror: {mirrored: number, unchanged: number, deleted: string[], forkedPreserved: string[], carriersPreserved: string[], skippedCarriers: string[], skippedLinks: string[], skippedSessionIds: string[]}}>}
   */
  // 镜像（不提交）：把 sessionRoot 的字节镜像进工作树 + 确保设备文件。
  // 只写本地工作树，不触网、不确认、不提交。调用方已持有 schedule 锁，
  // 这里绝不能再走 #schedule。
  async #mirrorOnly(deleteMissing = true, skipSessionIds = []) {
    ...
    if (this.#deps.prepareMirrorMetadata !== undefined) {
      const prepared = await this.#deps.prepareMirrorMetadata({ repoDir: this.#deps.repoDir, mirrorDir: MIRROR_DIR })
      metadataChanged = typeof prepared === 'boolean' ? prepared : prepared?.changed === true
    }
    return { deviceWritten, mirror, metadataChanged }
  }

  /** Persisted inbound sessions that must not be mirrored/pushed from stale live state. */
  async #deferredSessionIds() {
    if (this.#deps.getDeferredSessionIds === undefined) return []
    const ids = await this.#deps.getDeferredSessionIds()
    return [...new Set(ids.map(String))].sort()
  }

  /** Fail closed while newer inbound bytes are waiting to replace a live Session. */
  async #assertPushAllowed() {
    const deferred = await this.#deferredSessionIds()
    if (deferred.length === 0) return
    throw new Error(
      `push blocked: ${deferred.length} session(s) have deferred inbound state; restart DSH and let startup pull finish before opening or continuing those sessions`,
    )
  }
  ...
  async #restoreLiveStore() {
    ...
    // Metadata may depend on newly materialized Session persistence (archives
    // do), but host reconciliation must observe that metadata before it
    // creates/attaches workspaces for restored Sessions.
    if (this.#deps.applyMirrorMetadata !== undefined) {
      await this.#deps.applyMirrorMetadata({ repoDir: this.#deps.repoDir, mirrorDir: MIRROR_DIR })
    }
  ...
  async #mirrorAndCommit(message, deleteMissing = true, skipSessionIds = []) {
    const { deviceWritten, mirror, metadataChanged } = await this.#mirrorOnly(deleteMissing, skipSessionIds)
    if (deviceWritten || metadataChanged || mirror.mirrored > 0 || mirror.deleted.length > 0) {
  ...
  status() {
    return this.#schedule(async () => {
      try {
        ...
        await this.#mirrorOnly(false, await this.#deferredSessionIds())
      } catch {
  ...
  pull(opts = {}) {
    return this.#schedule(async () => {
      try {
        ...
        await this.#deps.git.bootstrap()
        const deferred = await this.#deferredSessionIds()
        await this.#mirrorAndCommit('dsh-session-sync: local checkpoint before pull', false, deferred)
