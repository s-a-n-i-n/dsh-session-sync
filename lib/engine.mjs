...
export class SyncEngine {
  ...
  async #mirrorAndCommit(message, opts = {}) {
    const { deviceWritten, mirror } = await this.#mirrorOnly(opts.deleteMissing ?? true)
  ...
  pull(opts = {}) {
    return this.#schedule(async () => {
      try {
        ...
        await fs.mkdir(this.#deps.repoDir, { recursive: true })
        await this.#deps.git.bootstrap()
        await this.#mirrorAndCommit('dsh-session-sync: local checkpoint before pull', { deleteMissing: false })
