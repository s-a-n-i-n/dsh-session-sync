...
export class EncryptedBackend {
  ...
  #onRestored
  /** @type {((ctx: {repoDir: string, mirrorDir: string}) => boolean|{changed?: boolean}|Promise<boolean|{changed?: boolean}> )|undefined} */
  #prepareMirrorMetadata
  /** @type {((ctx: {repoDir: string, mirrorDir: string}) => void|Promise<void>)|undefined} */
  #applyMirrorMetadata
  ...
  constructor(deps) {
    ...
    this.#getDeferredSessionIds = deps.getDeferredSessionIds
    this.#onRestored = deps.onRestored
    this.#prepareMirrorMetadata = deps.prepareMirrorMetadata
    this.#applyMirrorMetadata = deps.applyMirrorMetadata
    ...
    this.#plain = new SyncEngine({
      ...
      getDeferredSessionIds: deps.getDeferredSessionIds,
      onRestored: deps.onRestored,
      prepareMirrorMetadata: deps.prepareMirrorMetadata,
      applyMirrorMetadata: deps.applyMirrorMetadata,
      logger: deps.logger,
    })
  }
  ...
  async #mirrorOnly(deleteMissing = true, skipSessionIds = []) {
    ...
    if (this.#prepareMirrorMetadata !== undefined) {
      await this.#prepareMirrorMetadata({ repoDir: this.#repoDir, mirrorDir: MIRROR_DIR })
    }
    return mirror
  }

  async #deferredSessionIds() {
    if (this.#getDeferredSessionIds === undefined) return []
    const ids = await this.#getDeferredSessionIds()
  ...
  async #finishPull(result) {
    ...
    if (this.#applyMirrorMetadata !== undefined) {
      await this.#applyMirrorMetadata({ repoDir: this.#repoDir, mirrorDir: MIRROR_DIR })
    }
    return {
      ...result,
